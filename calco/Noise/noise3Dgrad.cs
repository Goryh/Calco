...
namespace calco
{
    ...
    {
        public static float snoise(float3a v, out float3a gradient)
        {
            float2 C = float2(1.0f / 6.0f, 1.0f / 3.0f);
            float4 D = float4(0.0f, 0.5f, 1.0f, 2.0f);

            // First corner
            float3a i = floor(v + dot(v, C.yyya));
            float3a x0 = v - i + dot(i, C.xxxa);

            // Other corners
            float3a g = step(x0.yzx, x0.xyz);
            float3a l = 1.0f - g;
            float3a i1 = min(g.xyz, l.zxy);
            float3a i2 = max(g.xyz, l.zxy);

            //   x0 = x0 - 0.0 + 0.0 * C.xxx;
            //   x1 = x0 - i1  + 1.0 * C.xxx;
            //   x2 = x0 - i2  + 2.0 * C.xxx;
            //   x3 = x0 - 1.0 + 3.0 * C.xxx;
            float3a x1 = x0 - i1 + C.xxxa;
            float3a x2 = x0 - i2 + C.yyya; // 2.0*C.x = 1/3 = C.y
            float3a x3 = x0 - D.yyya; // -1.0+3.0*C.x = -0.5 = -D.y
            ...
            float3a p0 = float3a(a0.xy, h.x);
            float3a p1 = float3a(a0.zw, h.y);
            float3a p2 = float3a(a1.xy, h.z);
            float3a p3 = float3a(a1.zw, h.w);
