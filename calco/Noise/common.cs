...
namespace calco
{
    ...
    public static partial class noise
    {
        ...
        static float3 mod289(float3a x){ return x - floor(x * (1.0f / 289.0f)) * 289.0f; }
        static float4 mod289(float4 x) { return x - floor(x * (1.0f / 289.0f)) * 289.0f; }

        // Modulo 7 without a division
        static float3 mod7(float3a x){ return x - floor(x * (1.0f / 7.0f)) * 7.0f; }
        ...
        static float3 permute(float3a x){ return mod289((34.0f * x + 1.0f) * x); }
        ...
        static float3 fade(float3a t){ return t*t*t*(t*(t*6.0f-15.0f)+10.0f); }
        static float4 fade(float4 t) { return t*t*t*(t*(t*6.0f-15.0f)+10.0f); }

        static float4 grad4(float j, float4 ip)
        {
            float3a pxyz = floor(frac(float3a(j) * ip.xyza) * 7.0f) * ip.z - 1.0f;
            float  pw   = 1.5f - dot(abs(pxyz), float3c.one);
            float4 p = float4(pxyz, pw);
            float4 s = float4(p < 0.0f);
            p.xyz = p.xyza + (s.xyza*2.0f - 1.0f) * s.w;
            return p;
        }
