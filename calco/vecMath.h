...
{
	// s = 1.0f / Dot(q, q);
	Vec s = vec(vecRecip1(vecDot4(q, q)));
...
FORCEINLINE void vecNormalizingQuatToMatrix(Vec& xAxis, Vec& yAxis, Vec& zAxis, Vec q)
{
	...
	Vec xyz_2 = vecMul(vecAdd(q, q), vec(vecRecip1(vecDot4(q, q))));
...
FORCEINLINE Vec vecMathMatrixToQuaternion(Vec u, Vec v, Vec w)
{
	...
	// do normalization
	Vec invNorm = vec(vecRecipSqrt1(vecDot4(res, res)));
										...
										Vec inX, Vec inY, Vec inZ)
{
	...
	Vec recipDet = vec(vecRecip1(det));
