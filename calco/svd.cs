...
namespace calco
{
    ...
    static class svd
    {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        static void condNegSwap(bool c, ref float3a x, ref float3a y)
        {
            var tmp = -x;
            x = math.select(x, y, c);
            y = math.select(y, tmp, c);
        ...
        static quaternion condNegSwapQuat(bool c, quaternion q, float4 mask)
        {
            ...
            return math.mul(q, math.select(quaternionc.identity.value, mask * halfSqrt2, c));
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        static void sortSingularValues(ref float3ax3 b, ref quaternion v)
        {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        static quaternion givensQRFactorization(float3ax3 b, out float3ax3 r)
        {
            var u = qrGivensQuat(math.float2(b.c0.x, b.c0.y), math.float4(0f, 0f, 1f, 1f));
            var qmt = math.float3ax3(math.conjugate(u));
            r = math.mul(qmt, b);

            var q = qrGivensQuat(math.float2(r.c0.x, r.c0.z), math.float4(0f, -1f, 0f, 1f));
            u = math.mul(u, q);
            qmt = math.float3ax3(math.conjugate(q));
            r = math.mul(qmt, r);
        ...
        static quaternion jacobiIteration(ref float3ax3 s, int iterations = 5)
        {
            float3ax3 qm;
            quaternion q;
            quaternion v = quaternionc.identity;
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        static float3 singularValuesDecomposition(float3ax3 a, out quaternion u, out quaternion v)
        {
            u = quaternionc.identity;
            v = quaternionc.identity;

            var s = math.mul(math.transpose(a), a);
            v = jacobiIteration(ref s);
            var b = math.float3ax3(v);
            b = math.mul(a, b);
        ...
        static float3 rcpsafe(float3 x, float epsilon = k_EpsilonRCP) =>
            math.select(math.rcp(x), (float3)float3c.zero, math.abs(x) < epsilon);

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        internal static float3ax3 svdInverse(float3ax3 a)
        {
            var e = singularValuesDecomposition(a, out var u, out var v);
            var um = math.float3ax3(u);
            var vm = math.float3ax3(v);

            return math.mul(vm, math.scaleMul(rcpsafe(e, k_EpsilonDeterminant), math.transpose(um)));
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        internal static quaternion svdRotation(float3ax3 a)
        {
