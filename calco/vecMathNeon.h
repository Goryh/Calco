...
FORCEINLINE Vec1 vecDiv1(Vec1 r1, Vec1 r2)
{
	return r1 / r2;
}
...
FORCEINLINE Vec1 vecRecipNoNewtonRaphson1(Vec1 v)
{
	// Warning: VRECPE is only good for about 8 bits of precision on most ARM chips (compared to 12 bits on x86)
...
FORCEINLINE Vec1 vecRecipEst1(Vec1 v)
{
	// Do one iteration of Newton-Raphson, because VRECPE is less accurate than x86 equivalent
	const Vec1 estimate = vecRecipNoNewtonRaphson1(v);

	// NEON has an intrinsic that does one step of Newton-Raphson
	const Vec1 step = vrecpss_f32(estimate, v);

	return vecMul1(estimate, step);
}
...
FORCEINLINE Vec1 vecRecip1(Vec1 v)
{
	// Do two iterations of Newton-Raphson
	const Vec1 estimate = vecRecipEst1(v);
...
FORCEINLINE Vec1 vecSel1(Vec1 falseValue, Vec1 trueValue, Vec1 mask)
{
